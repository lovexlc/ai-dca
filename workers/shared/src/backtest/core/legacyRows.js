...
export function buildPremiumSpreadInputFromLegacyRows(rows = [], symbols = {}) {
  ...
  const historyByCode = { [highCode]: [], [lowCode]: [] };
  const navHistoryByCode = { [highCode]: [], [lowCode]: [] };
  ...
  (Array.isArray(rows) ? rows : []).forEach((row, index) => {
    ...
    if (highClose != null) historyByCode[highCode].push({ t, date, c: highClose, bidPrice: positiveNumber(row?.sellBid), askPrice: positiveNumber(row?.sellAsk) });
    if (lowClose != null) historyByCode[lowCode].push({ t, date, c: lowClose, bidPrice: positiveNumber(row?.buyBid), askPrice: positiveNumber(row?.buyAsk) });
