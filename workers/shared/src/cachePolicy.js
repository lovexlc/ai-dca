/*
 * Generic cache-envelope and freshness policy shared by Workers.
 *
 * This module intentionally has no Worker-specific imports. Product-specific
 * modules may re-export it for compatibility, but the source of truth stays
 * in workers/shared.
 */
...
export function createCacheEnvelope({
  ...
  if (!String(key || '').trim() || !String(source || '').trim() || !Number.isFinite(fetchedTimestamp)
    ...
    || staleTimestamp < validTimestamp || payload == null) return null;
  return {
...
export function validateCacheEnvelope(envelope, { key = '', source, payloadValidator } = {}) {
