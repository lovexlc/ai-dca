...
async function actualDelivery(env, notification, options, settings, clientId) {
  ...
  if (wants(targets, 'bark')) jobs.push((async () => ({ ...(await sendBarkNotification({ ...notification, url: notification.url || notification.detailUrl || '', deviceKey: text(settings.barkDeviceKey, 512) })), configKey: `bark-client:${clientId}`, configType: 'bark-client', configId: clientId, configLabel: label ? `Bark · ${label}` : 'Bark' }))());
  if (wants(targets, 'serverchan3')) { const config = settings.serverChan3 || {}; jobs.push((async () => ({ ...(await sendServerChan3Notification({ ...notification, uid: text(config.uid, 240), sendKey: text(config.sendKey, 512) })), configKey: `serverchan3-client:${clientId}`, configType: 'serverchan3-client', configId: clientId, configLabel: label ? `Server酱³ · ${label}` : 'Server酱³' }))()); }
  if (wants(targets, 'email')) { const email = normalizeEmailConfig(settings.email || {}); jobs.push((async () => ({ ...(await sendVerifiedEmailNotification({ ...notification, email, detailUrl: notification.detailUrl || notification.url || '' }, env)), configKey: `email-client:${clientId}`, configType: 'email-client', configId: clientId, configLabel: email.address ? `Email · ${maskEmailAddress(email.address)}` : 'Email' }))()); }
  const settled = await Promise.allSettled(jobs); const results = settled.map((item, index) => item.status === 'fulfilled' ? item.value : { channel: ['bark', 'serverchan3', 'email'][index] || 'unknown', status: 'failed', detail: item.reason instanceof Error ? item.reason.message : String(item.reason || '通知发送失败') });
  ...
  const delivered = results.some((item) => item.status === 'delivered' || (item.channel === 'pc' && item.status === 'queued'));
