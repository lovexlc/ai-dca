...
export default {
  async fetch(request, env, ctx) {
    ...
    try {
      const authenticatedRequest = await authenticateNotifyAccountRequest(request, env); const url = new URL(authenticatedRequest.url); const method = authenticatedRequest.method;
      ...
      if (method === 'POST' && url.pathname === '/api/notify/email/send-code') { const queuedRequest = authenticatedRequest.clone(); return await deferAccountOperation(queuedRequest, env, ctx, () => handleFastEmailRoute(queuedRequest.clone(), env, url.pathname), { type: 'notify-email-code' }); }
      ...
      if (method === 'POST' && url.pathname === '/api/notify/switch/run') { const queuedRequest = authenticatedRequest.clone(); return await deferAccountOperation(queuedRequest, env, ctx, () => notifyWorker.fetch(queuedRequest.clone(), env, ctx), { type: 'notify-switch-run' }); }
      ...
      if (method === 'POST' && url.pathname === '/api/notify/sync') { const queuedRequest = authenticatedRequest.clone(); return await deferAccountOperation(queuedRequest, env, ctx, () => handleFastSync(queuedRequest.clone(), env, ctx), { type: 'notify-sync' }); }
      if (method === 'POST' && url.pathname === '/api/notify/test') { const queuedRequest = await stripDeviceIdentityFromAccountTestRequest(authenticatedRequest); return await deferAccountOperation(queuedRequest, env, ctx, () => handleDirectAccountTest(queuedRequest.clone(), env), { type: 'notify-test' }); }
      return await notifyWorker.fetch(authenticatedRequest, env, ctx);
    } catch (error) {
      if (error instanceof NotifyAccountAuthError || error instanceof AccountSettingsError) { const payload = { error: error.message, code: error.code }; if (error.code === 'CHANNEL_REBIND_REQUIRED') payload.canRebind = true; if (error.code === 'CHANNEL_BINDING_MISMATCH') payload.canRebind = false; return jsonResponse(payload, { status: error.status, origin }); }
      const status = Number(error?.status) || 0; if (status >= 400 && status < 500) return jsonResponse({ error: error instanceof Error ? error.message : '通知请求无效', ...(error?.code ? { code: String(error.code) } : {}) }, { status, origin });
