...
function timedResponse(payload, request, startedAt, queryMs) {
  ...
  headers.set('server-timing', `query;dur=${queryMs}, total;dur=${totalMs}`);
...
export async function handleAccountStatus(request, env) {
  ...
  const serverConfigured = Boolean(text(state.server?.uid) && text(state.server?.sendKey));
  ...
  const data = {
    ...
    channels: {
      ...
      serverChan3: { uid: text(state.server?.uid), sendKeyMasked: maskServerChan3SendKey(state.server?.sendKey || ''), configured: serverConfigured },
    ...
    rules: {
      planRuleCount: Number(state.meta?.counts?.planRuleCount) || 0,
      dcaRuleCount: Number(state.meta?.counts?.dcaRuleCount) || 0,
      totalRuleCount: Number(state.meta?.counts?.totalRuleCount) || 0
    },
  ...
  console.log('[notify-account-status-timing]', JSON.stringify({ queryMs, totalMs: Date.now() - startedAt }));
...
export async function handleAccountEvents(request, env) {
  ...
  const account = accountOf(request);
  const queryStartedAt = Date.now();
  const result = await env.SYNC_DB.prepare(`SELECT payload FROM ${TABLE}
    WHERE owner_user_id = ? AND record_type = 'event'
  ...
  const queryMs = Date.now() - queryStartedAt;
  console.log('[notify-account-events-timing]', JSON.stringify({ rows: events.length, queryMs, totalMs: Date.now() - startedAt }));
  return timedResponse({ events }, request, startedAt, queryMs);
}
