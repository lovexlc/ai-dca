export function normalizeServerChan3Config(config = {}) {
  return { uid: String(config?.uid || '').trim(), sendKey: String(config?.sendKey || '').trim() };
}
...
function singleLine(value = '') { return normalizeText(value).replace(/\s+/g, ' '); }
function truncateText(value = '', maxLength = MAX_SHORT_LENGTH) {
  const normalized = singleLine(value);
  return normalized.length <= maxLength ? normalized : `${normalized.slice(0, Math.max(maxLength - 1, 0))}…`;
}
function markdownTableCell(value = '') { return singleLine(value).replace(/\|/g, '\\|'); }
...
function resolveServerChanTags(notification = {}) {
  ...
  const rawTags = Array.isArray(notification.tags) ? notification.tags : String(notification.tags || '').split('|');
  const tags = [DEFAULT_TAG, EVENT_TAGS[eventType] || '', ...rawTags]
...
function buildServerChanMarkdown(notification = {}) {
  ...
  if (rows.length) {
    lines.push(['| 项目 | 内容 |', '| --- | --- |', ...rows.map(([key, value]) => `| ${markdownTableCell(key)} | ${markdownTableCell(value)} |`)].join('\n'));
  }
  if (bodyMd) lines.push(bodyMd);
  else if (body) lines.push(body);
  if (detailUrl) lines.push(`[打开 AI-DCA 查看详情](${detailUrl})`);
  return lines.join('\n\n');
}
...
export function buildServerChan3MessagePayload(notification = {}) {
  ...
  const shortSource = normalizeText(notification.short) || normalizeText(notification.summary) || title;
  return {
    title,
    desp: markdown,
...
export async function sendServerChan3Notification({ uid = '', sendKey = '', ...notification } = {}) {
  ...
  if (!normalizedUid || !normalizedSendKey) {
    return { channel: 'serverchan3', status: 'skipped', detail: '未配置 Server酱³ UID 或 SendKey' };
  }
  ...
  try {
    ...
    if (error?.name === 'TimeoutError' || error?.name === 'AbortError') {
      throw new Error(`Server酱³ 推送失败：上游 ${NOTIFY_PROVIDER_TIMEOUT_MS / 1000} 秒内未响应`);
    }
    throw error;
  }
  const rawText = await response.text();
  if (!response.ok) throw new Error(rawText || `Server酱³ 推送失败：状态 ${response.status}`);
  return { channel: 'serverchan3', status: 'delivered', detail: rawText || '已发送到 Server酱³' };
}
