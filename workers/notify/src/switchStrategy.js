...
export function computeSwitchSnapshot(config, priceMap, navByCode, computedAt) {
  ...
  function buildPremiumEntry(code) {
    ...
    return {
      ...
      highPoint: Number(priceMap?.[code]?.highPoint?.high ?? priceMap?.[code]?.highPoint?.yearHigh) || null,
      yearHigh: Number(priceMap?.[code]?.yearHigh) || null,
      historicalPercentile: Number.isFinite(Number(priceMap?.[code]?.historicalPercentile)) ? Number(priceMap?.[code]?.historicalPercentile) : null,
      turnover: Number(priceMap?.[code]?.turnover) || null,
  ...
  const byBenchmark = benchmarkCodes.map((benchmarkCode) => {
    ...
    return {
      ...
      benchmarkHigh: Number(priceMap?.[benchmarkCode]?.high) || null,
      benchmarkHighPoint: Number(priceMap?.[benchmarkCode]?.highPoint?.high ?? priceMap?.[benchmarkCode]?.highPoint?.yearHigh) || null,
      benchmarkYearHigh: Number(priceMap?.[benchmarkCode]?.yearHigh) || null,
      benchmarkHistoricalPercentile: Number.isFinite(Number(priceMap?.[benchmarkCode]?.historicalPercentile)) ? Number(priceMap?.[benchmarkCode]?.historicalPercentile) : null,
      benchmarkTurnover: Number(priceMap?.[benchmarkCode]?.turnover) || null,
