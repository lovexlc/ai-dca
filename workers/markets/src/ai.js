...
const DEEP_DIVE_PROMPT = `【深度模式】请直接输出最终答案，**严禁**在答案里出现任何关于「怎么思考 / 如何组织 / 检查一下 / 我需要 / 现在起草 / 接下来我」这类元话术或推理过程，也不要包含「思考」「thinking」「reasoning」「analysis」任一标签。只输出以下结构化结果，总长 500-900 字，使用 markdown：
...
- 「风险提示」：一句话。
请直接以「首段」开始输出正文，不要加「以下是我的回答 / 以下是分析」这类开场白。请结合「主题上下文」中的主题概要、相关新闻、以及「行情快照」上的代表性仓位，避免只重复标题。`;
