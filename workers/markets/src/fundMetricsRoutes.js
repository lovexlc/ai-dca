...
export function normalizeFundMetricFromQuote(code, quote, { cached = false, cachePolicy = '', primaryError = '', exchange = isExchangeTradedFund(code), fundKind = '' } = {}) {
  ...
  return {
    ...
    highPoint: quote?.highPoint && typeof quote.highPoint === 'object' ? quote.highPoint : null,
    yearHigh: firstPositiveNumber(quote?.yearHigh, quote?.highPoint?.high),
    historicalPercentile: [quote?.historicalPercentile, quote?.historyPercentile, quote?.percentile].map(Number).find(Number.isFinite) ?? null,
