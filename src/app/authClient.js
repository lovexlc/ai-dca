...
const SHA256_K = [
  0x428a2f98, 0x71374491, 0xb5c0fbcf, 0xe9b5dba5, 0x59f111f1, 0x923f82a4, 0xab1c5ed5,
  0xd807aa98, 0x12835aa6, 0x243185be, 0x550c7dc3, 0x72be5d74, 0x80deb1fe, 0x9bdc06a7, 0xc19bf174,
  0xe49b69c1, 0xefbe4786, 0x0fc19dc6, 0x2de92c6f, 0x4a7484aa, 0x5cb0a9dc, 0x76f988da,
  0x983e5152, 0xa831c66d, 0xb00327c8, 0xbf597fc7, 0xc6e00bf3, 0x06ca6351, 0x14292967,
  0x27b70a85, 0x2e1b2138, 0x4d2c6dfc, 0x650a7354, 0x766a0abb, 0x81c2c92e, 0x92722c85,
  0xa81a664b, 0xc24b8b70, 0xc76c51a3, 0xd192e819, 0xf40e3585, 0x106aa070,
  0x19a4c116, 0x1e376c08, 0x2748774c, 0x34c0c0b3, 0x391c0cb3, 0x4ed8aa4a, 0x5b9cca4f,
  0x748f82ee, 0x78a5636f, 0x84c87814, 0x8cc70208, 0x90befffa, 0xa4506ceb, 0xbef9a3f7, 0xc67178f2
];

function getSyncBase() {
  if (typeof window !== 'undefined' && window.__AI_DCA_SYNC_BASE__) return String(window.__AI_DCA_SYNC_BASE__).replace(/\/$/, '');
...
function sha256HexFallback(text) {
  ...
  for (let offset = 0; offset < paddedLength; offset += 64) {
    ...
    for (let i = 0; i < 64; i += 1) { const s1 = rightRotate(e, 6) ^ rightRotate(e, 11) ^ rightRotate(e, 25); const ch = (e & f) ^ (~e & g); const temp1 = (h + s1 + ch + SHA256_K[i] + w[i]) >>> 0; const s0 = rightRotate(a, 2) ^ rightRotate(a, 13) ^ rightRotate(a, 22); const maj = (a & b) ^ (a & c) ^ (b & c); const temp2 = (s0 + maj) >>> 0; h = g; g = f; f = e; e = (d + temp1) >>> 0; d = c; c = b; b = a; a = (temp1 + temp2) >>> 0; }
...
async function sha256Hex(text, cryptoLike = globalThis.crypto) {
  ...
  if (typeof digestFn === 'function') { const digest = await digestFn.call(cryptoLike.subtle, 'SHA-256', bytes); return Array.from(new Uint8Array(digest), (byte) => byte.toString(16).padStart(2, '0')).join(''); }
  return sha256HexFallback(text);
}
async function passwordHash(username, password) { return sha256Hex(`${String(username || '').trim().toLowerCase()}:${String(password || '')}`); }
...
async function requestSync(path, { token = '', ...init } = {}) { const headers = { 'content-type': 'application/json; charset=utf-8', ...(init.headers || {}) }; if (token) headers.authorization = `Bearer ${token}`; const response = await fetch(`${getSyncBase()}${path}`, { ...init, headers }); const data = await readJson(response); if (!response.ok) { const error = new Error(data?.message || data?.error || `请求失败：HTTP ${response.status}`); error.status = response.status; error.data = data; error.response = response; throw error; } return data; }
...
export async function registerCloudAccount({ username, password }) {
  ...
  const data = await requestSync('/auth/register', { method: 'POST', body: JSON.stringify({ username: normalized, passwordHash: await passwordHash(normalized, password) }) });
...
export async function loginCloudAccount({ username, password }) {
  const normalized = String(username || '').trim().toLowerCase();
  const data = await requestSync('/auth/login', { method: 'POST', body: JSON.stringify({ username: normalized, passwordHash: await passwordHash(normalized, password) }) });
  const session = saveCloudSession(data);
...
export async function rollbackCloudBackupVersion(version, { baseVersion } = {}, session = loadCloudSession()) {
  if (remoteAuthorityEnabled()) throw legacyAuthorityError();
  if (!session?.accessToken) throw new Error('请先登录账户');
  return requestSync('/versions/rollback', { method: 'POST', token: session.accessToken, body: JSON.stringify({ version: Number(version), baseVersion: Number(baseVersion) }) });
}
