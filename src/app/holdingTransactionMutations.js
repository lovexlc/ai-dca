...
export async function persistHoldingTransactionMutation(nextState, { kind, label, deletedIds = [], setLedger } = {}) {
  return runAccountUserAction({ kind, resource: 'holdings/ledger', label }, async () => {
    ...
    try {
      const syncResult = await pushHoldingTransactions({ session, force: true, deletedIds });
      return {
        cloudAttempted: true,
        failed: Array.isArray(syncResult?.failed) ? syncResult.failed : []
      };
    } catch (error) {
      return { cloudAttempted: true, failed: [{ message: error?.message || '云端同步失败' }] };
    }
  });
}
