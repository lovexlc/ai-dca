...
export function isCnMigrationNoticeHost(location = globalThis.location) {
  ...
  return host === 'cn.freebacktrack.tech'
    || host === 'localhost'
    || host === '127.0.0.1'
...
function OptionCard({ selected, danger = false, icon: Icon, title, description, onClick }) {
  return (
    <button
      type="button"
      className={cx(
        ...
        selected && !danger && 'border-indigo-500 bg-indigo-50/80 focus:ring-indigo-500',
...
export function AccountDataMigrationModal() {
  ...
  useEffect(() => {
    ...
    return () => window.removeEventListener(CLOUD_SYNC_SESSION_EVENT, handleSession);
  }, [refresh]);

  useEffect(() => { void refresh(session); }, [refresh, session?.accessToken, session?.userId, session?.username]);
  useEffect(() => {
    function handleMigration() { if (!result) void refresh(loadCloudSession()); }
    window.addEventListener(ACCOUNT_MIGRATION_EVENT, handleMigration);
    return () => window.removeEventListener(ACCOUNT_MIGRATION_EVENT, handleMigration);
  }, [refresh, result]);
  ...
  async function migrate() {
    ...
    setError('');
    try {
      const outcome = await runLegacyMigration({ securityPassword: password, useRemembered: true, overwrite: false });
  ...
  async function removeAll() {
    ...
    try {
      ...
      setResult({
        ...
        tone: 'slate',
        title: '账号数据已清空',
        description: '旧云端密文、新同步数据与本机业务数据均已删除，登录账户仍然保留。'
      });
      setPhase('success');
    } catch (deleteError) {
      setError(deleteError?.message || '删除未完成；在成功前不会清除本机数据。');
      setPhase('ready');
    }
  }
  ...
  return (
    <Dialog open={open} onOpenChange={(next) => { if (!next) dismiss(); }}>
      <DialogContent
        ...
        onEscapeKeyDown={(event) => { if (pending || busy) event.preventDefault(); else dismiss(); }}
      >
        {result ? (
          <div className="px-6 py-9 text-center sm:px-10">
            <span className={cx(
              'mx-auto flex h-16 w-16 items-center justify-center rounded-2xl',
              result.tone === 'emerald' ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-100 text-slate-700'
            )}>
              <ResultIcon className="h-8 w-8" aria-hidden="true" />
            </span>
            <DialogTitle className="mt-5 text-2xl font-bold text-slate-950">{result.title}</DialogTitle>
            <DialogDescription className="mx-auto mt-2 max-w-md leading-6 text-slate-500">{result.description}</DialogDescription>
            <button type="button" className="mt-7 h-11 rounded-xl bg-slate-950 px-5 text-sm font-bold text-white" onClick={() => window.location.reload()}>
              重新加载并继续
            </button>
          </div>
        ) : (
          <>
            <header className="relative flex-none overflow-hidden bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-4 py-4 text-white sm:px-7 sm:pb-7 sm:pt-6">
              ...
              {!pending && !busy ? (
                <button type="button" className="absolute right-3 top-3 z-10 flex h-9 w-9 items-center justify-center rounded-full text-white/60 hover:bg-white/10 hover:text-white" onClick={dismiss} aria-label="关闭数据安全更新提醒">
              ...
              <div className="relative flex items-start gap-3 pr-9 sm:gap-4">
                ...
                <div className="min-w-0">
                  <div className="text-[11px] font-bold tracking-[0.18em] text-indigo-200">数据安全更新</div>
            ...
            <main className="min-h-0 flex-1 overscroll-contain overflow-y-auto px-4 py-4 sm:px-7 sm:py-6">
              {phase === 'error' && !migration ? (
                <div className="flex min-h-52 flex-col items-center justify-center text-center">
                  ...
                  <div className="mt-1 text-xs leading-5 text-slate-500">{error}</div>
                  <button type="button" className="mt-5 flex h-10 items-center gap-2 rounded-xl border border-slate-200 px-4 text-sm font-semibold" onClick={() => void refresh(session)}>
                ...
                <div className="space-y-4 sm:space-y-5">
                  ...
                  <section className="flex items-center gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-3.5 py-3">
                    ...
                    <div>
                      ...
                      <div className="mt-0.5 text-xs text-slate-500">{legacySummary}</div>
                    </div>
                  </section>
                  ...
                  {showOptions ? (
                    <div className="grid gap-3 sm:grid-cols-3">
                      <OptionCard
                        ...
                        onClick={() => { setMode(pending ? 'migrate' : 'keep'); setError(''); }}
                      />
                      <OptionCard
                    ...
                    <section className="rounded-2xl border border-indigo-200 bg-indigo-50/60 p-3.5 sm:p-4">
                      {needsOriginalDevice ? (
                        <div className="flex gap-2 text-xs leading-5 text-indigo-900">
                          ...
                          <span>该备份由原设备密钥加密，请在原设备迁移，或选择清空数据。</span>
                        </div>
                      ) : (
                        <>
                          ...
                          {needsPassword ? (
                            <label className="mt-3 block text-xs font-semibold text-slate-700">
                              ...
                              <span className="relative mt-1.5 block">
                                <input className={cx(inputClass, 'h-11 pr-11')} type={passwordVisible ? 'text' : 'password'} value={password} onChange={(event) => setPassword(event.target.value)} autoComplete="off" placeholder="输入原安全密码（至少 8 位）" autoFocus />
                                <button type="button" className="absolute right-2 top-1/2 flex h-8 w-8 -translate-y-1/2 items-center justify-center rounded-lg text-slate-400" onClick={() => setPasswordVisible((value) => !value)} aria-label={passwordVisible ? '隐藏安全密码' : '显示安全密码'}>
                  ...
                  {mode === 'delete' ? (
                    <section className="rounded-2xl border border-red-200 bg-red-50 p-3.5 sm:p-4">
                      ...
                      <p className="mt-1 text-xs leading-5 text-red-800">登录账户会保留，但旧云端密文、同步资源、历史版本及本机业务数据都会删除。</p>
                      <label className="mt-3 block text-xs font-semibold text-red-900">
                        输入“{DELETE_TEXT}”确认
                        <input className={cx(inputClass, 'mt-1.5 h-11 border-red-200 bg-white')} value={deleteText} onChange={(event) => setDeleteText(event.target.value)} autoComplete="off" placeholder={DELETE_TEXT} />
                      </label>
                    </section>
                  ) : null}

                  {error ? <div className="rounded-xl border border-red-200 bg-red-50 px-3 py-2.5 text-xs leading-5 text-red-700" role="alert">{error}</div> : null}
                </div>
              )}
            </main>
            ...
            {phase !== 'loading' && !(phase === 'error' && !migration) ? (
              <footer className="flex flex-none flex-col-reverse gap-2 border-t border-slate-200 bg-slate-50/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 sm:flex-row sm:items-center sm:justify-between sm:px-7 sm:py-4">
                ...
                {mode === 'source' ? (
                  ...
                  <button type="button" className="flex h-11 items-center justify-center gap-2 rounded-xl bg-indigo-600 px-5 text-sm font-bold text-white disabled:opacity-50" onClick={() => void migrate()} disabled={busy || (pending && (!canMigrate || (needsPassword && password.length < 8)))}>
                    {phase === 'migrating' ? <Loader2 className="h-4 w-4 animate-spin" /> : pending ? <KeyRound className="h-4 w-4" /> : <CheckCircle2 className="h-4 w-4" />}
                    {phase === 'migrating' ? '正在迁移…' : pending ? '开始迁移' : '我已了解，继续使用'}
                  </button>
                )}
              </footer>
            ) : null}
