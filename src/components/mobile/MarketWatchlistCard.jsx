...
export function MarketWatchlistCard({ row, kline, selected = false, onClick, columns = [] }) {
  ...
  return (
    <button type="button" className={cx("market-mobile-card", selected && "is-selected")} onClick={() => onClick?.(row)} aria-pressed={selected}>
      ...
      <span className="market-mobile-card__metrics" aria-label="行情摘要">{visible.has("price") ? <span><small>{isOtc ? "最新净值" : "最新价"}</small><b>{displayPrice}</b></span> : null}{visible.has("changePercent") ? <span><small>今日涨跌幅</small><b className={tone}>{formatPercent(row?.changePercent)}</b></span> : null}{visible.has("change") ? <span><small>今日涨跌额</small><b className={tone}>{displayChange}</b></span> : null}{visible.has("updatedAt") ? <span><small>更新时间</small><b>{updateLabel}</b></span> : null}</span>
    </button>
  );
}
