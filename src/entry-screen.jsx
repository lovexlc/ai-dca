...
function runWhenIdle(callback, { timeout = 2500, delayMs = 0 } = {}) {
  ...
  if (delayMs > 0) window.setTimeout(scheduleIdle, delayMs);
  else scheduleIdle();
}
...
function startPostHogWhenIdle() {
  runWhenIdle(() => { initPostHog(); }, { timeout: 3500, delayMs: 30000 });
}
  ...
  runWhenIdle(async () => {
    try {
      const [{ readNotifyAccountUsername, readNotifyClientConfig }, { startNotifyRealtime }] = await Promise.all([
        import("./app/notifySync.js"), import("./app/notifyWsClient.js")
      ]);
      ...
      const ensureMarketDataRealtime = () => {
        if (!marketDataStarted) {
          ...
          if (typeof window !== "undefined") window.__aiDcaDisconnectNotifyWs = realtimeClient.disconnect;
        }
