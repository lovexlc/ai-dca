...
function loadAdsScriptWhenIdle() {
  runWhenIdle(
    () => {
      ...
      script.src =
        "https://pagead2.googlesyndication.com/pagead/js/adsbygoogle.js?client=ca-pub-1376743188081698";
...
function startNotifyRealtimeWhenIdle() {
  runWhenIdle(
    async () => {
      try {
        const [
          { readNotifyAccountUsername, readNotifyClientConfig },
          { startNotifyRealtime },
        ] = await Promise.all([
        ...
        if (
          ...
          typeof window.__aiDcaDisconnectNotifyWs === "function"
        ) {
          try {
            window.__aiDcaDisconnectNotifyWs();
          } catch {
            /* ignore */
          }
        }
          ...
          onStatusChange: (status) => {
            if (typeof window !== "undefined") {
              ...
              window.dispatchEvent(
                new CustomEvent("ai-dca-notify-ws-status", {
                  detail: { status },
                }),
              );
            }
        ...
        const ensureMarketDataRealtime = () => {
          if (!marketDataStarted) {
            ...
            realtimeClient = startNotifyRealtime({
              ...realtimeOptions,
        ...
        if (typeof window !== "undefined") {
          ...
          window.__aiDcaSubscribeMarketData = (symbols, options) =>
            ensureMarketDataRealtime().subscribeMarketData(symbols, options);
        }
      } catch {
        // 通知是辅助功能，启动失败不影响主页面
      }
