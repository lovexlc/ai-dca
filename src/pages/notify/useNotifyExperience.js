...
import { promptNotifyConfigSuccess, promptNotifyTestSuccess } from './notifyConversionPrompts.js';
...
import { useNotifyAlertRules } from './useNotifyAlertRules.js';
import { useSwitchNotifyRules } from './useSwitchNotifyRules.js';
import { navigateWorkspace } from './workspaceNavigation.js';
import { buildNotifyMeta } from './notifyAnalyticsMeta.js';
...
export function useNotifyExperience({ embedded = false } = {}) {
  ...
  const [notifyWsStatus, setNotifyWsStatus] = useState(() => {
    if (typeof window !== 'undefined') return window.__aiDcaNotifyWsStatus || 'idle';
    return 'idle';
  });
  ...
  const notifyMeta = () => buildNotifyMeta({
    embedded,
    notifyPlatform,
    barkConfigured,
  ...
  useEffect(() => {
    function handleWsStatusChange(event) {
      ...
      if (newStatus) setNotifyWsStatus(newStatus);
    }
  ...
  useEffect(() => {
    const timer = window.setInterval(() => setEventsTick((value) => value + 1), 60 * 1000);
  ...
  async function refreshNotifyData({ serverChan3Fallback = null } = {}) {
    ...
    const nextStatus = fallbackConfigured && !statusPayload?.setup?.serverChan3?.configured
      ? {
        ...statusPayload,
        configured: { ...(statusPayload?.configured || {}), serverChan3: true },
        setup: {
          ...(statusPayload?.setup || {}),
          serverChan3: { ...(statusPayload?.setup?.serverChan3 || {}), ...serverChan3Fallback, configured: true }
        }
      }
      : statusPayload;
  ...
  async function handleSaveNotifyConfig() {
    ...
    setNotifyMessage('');
    const startedAt = Date.now();
    trackFeatureEvent('notify', 'bark_save_start', { ...notifyMeta(), inputLength: String(notifyConfig.barkDeviceKey || '').length });
    try {
      const parsedBarkKey = parseBarkInput(notifyConfig.barkDeviceKey);
      if (!parsedBarkKey) throw new Error('请粘贴 Bark 完整链接或 Device Key');
      await saveNotifySettings({ barkDeviceKey: parsedBarkKey });
      persistNotifyClientConfig({ barkDeviceKey: parsedBarkKey, _hasServerChan3: serverChan3Configured, _hasPC: webNotifySupported && webNotifyPermission === 'granted' && webNotifyEnabled });
      ...
      trackActionResult('notify', 'bark_save', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'bark_save', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  async function handleSaveServerChan3Config() {
    ...
    setNotifyMessage('');
    const startedAt = Date.now();
    trackFeatureEvent('notify', 'serverchan3_save_start', { ...notifyMeta(), hasUid: Boolean(notifyConfig.serverChan3Uid), hasSendKey: Boolean(notifyConfig.serverChan3SendKey) });
    try {
      const uid = String(notifyConfig.serverChan3Uid || '').trim();
      const sendKey = String(notifyConfig.serverChan3SendKey || '').trim();
      if (!uid || (!sendKey && !serverChan3Configured)) throw new Error('请填写 Server酱³ UID 和 SendKey');
      ...
      setNotifyStatus((current) => ({
        ...(current || {}),
        configured: { ...(current?.configured || {}), serverChan3: true },
        setup: { ...(current?.setup || {}), serverChan3: savedServerChan3 }
      }));
      ...
      trackActionResult('notify', 'serverchan3_save', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'serverchan3_save', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
      setIsSavingSettings(false);
    }
  }
  ...
  async function handleTestBarkNotify() {
    ...
    setNotifyMessage('');
    const startedAt = Date.now();
    trackFeatureEvent('notify', 'bark_test_start', { ...notifyMeta(), inputLength: String(notifyConfig.barkDeviceKey || '').length });
    try {
      const parsedBarkKey = parseBarkInput(notifyConfig.barkDeviceKey);
      if (!parsedBarkKey) throw new Error('请先填写 iOS Bark 链接或 Device Key');
      ...
      trackActionResult('notify', 'bark_test', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'bark_test', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  async function handleTestServerChan3Notify() {
    ...
    setNotifyMessage('');
    const startedAt = Date.now();
    trackFeatureEvent('notify', 'serverchan3_test_start', { ...notifyMeta(), hasUid: Boolean(notifyConfig.serverChan3Uid), hasSendKey: Boolean(notifyConfig.serverChan3SendKey) });
    try {
      const uid = String(notifyConfig.serverChan3Uid || '').trim();
      const sendKey = String(notifyConfig.serverChan3SendKey || '').trim();
      if (!uid || (!sendKey && !serverChan3Configured)) throw new Error('请填写 Server酱³ UID 和 SendKey 后再测试');
      const payload = await sendNotifyTest({
        clientId: notifyConfig.notifyClientId,
        clientLabel: notifyConfig.notifyClientLabel,
      ...
      trackActionResult('notify', 'serverchan3_test', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'serverchan3_test', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
      setTestingNotifyChannel('');
    }
  }
  ...
  async function handleToggleHoldingsRule(nextEnabled) {
    ...
    setNotifyMessage('');
    const startedAt = Date.now();
    trackFeatureEvent('notify', 'holdings_rule_toggle_start', { ...notifyMeta(), nextEnabled });
    try {
      const digest = buildLatestHoldingsDigest();
      const payload = await saveHoldingsNotifyRule({ enabled: nextEnabled, digest });
      setHoldingsRule({ enabled: Boolean(payload?.enabled), digest: payload?.digest || digest || null, updatedAt: String(payload?.updatedAt || new Date().toISOString()) });
      setNotifyMessage(nextEnabled ? '持仓当日收益提醒已启用。' : '持仓当日收益提醒已关闭。');
      showActionToast(nextEnabled ? '启用持仓提醒' : '关闭持仓提醒', 'success');
      trackActionResult('notify', 'holdings_rule_toggle', 'success', { ...notifyMeta(), nextEnabled, hasDigest: Boolean(digest), digestItemCount: Array.isArray(digest?.items) ? digest.items.length : 0, durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'holdings_rule_toggle', 'error', { ...notifyMeta(), nextEnabled, durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  async function handleSyncHoldingsDigest() {
    ...
    try {
      ...
      trackActionResult('notify', 'holdings_digest_sync', 'success', { ...notifyMeta(), hasDigest: Boolean(digest), digestItemCount: Array.isArray(digest?.items) ? digest.items.length : 0, durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'holdings_digest_sync', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  async function handleTestHoldingsNotify() {
    ...
    try {
      ...
      showActionToast('测试通知', 'success', { description: '已发送「持仓总览」样式的测试推送。' });
      trackActionResult('notify', 'holdings_test', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'holdings_test', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  async function handleSyncRules() {
    ...
    try {
      ...
      await refreshNotifyEvents();
      trackActionResult('notify', 'trade_rules_sync', 'success', { ...notifyMeta(), durationMs: Date.now() - startedAt });
      ...
      trackActionResult('notify', 'trade_rules_sync', 'error', { ...notifyMeta(), durationMs: Date.now() - startedAt, errorMessage: message });
    } finally {
  ...
  const rulesLastSyncedLabel = rulesLastSyncedAt ? formatEventTimeLabel(rulesLastSyncedAt) : '本次会话尚未同步';
  return {
    availablePlatforms,
    pcFeaturesAvailable,
    serverChan3Configured,
