...
export function SwitchStrategyRuleModal({
  ...
  const [autoFeeTotal, setAutoFeeTotal] = useState('20');
              ...
              {autoPanelOpen ? (
                <div className="mt-3 border-t border-indigo-100 pt-3">
                  <div className="grid gap-3 sm:grid-cols-2">
                    ...
                    <Field label="预计单次切换费用" rightLabel="元" helper="包含卖出、买入佣金及其他费用">
                      <TextInput
                        value={autoFeeTotal}
                        inputMode="decimal"
                        placeholder="20"
                        onChange={(event) => {
                          setAutoFeeTotal(sanitizePctInput(event.target.value));
                          setAutoError('');
                          setAutoMessage('');
                        }}
                      />
                  ...
                  {autoMessage ? <p role="status" className="mt-2 text-[11px] font-bold leading-5 text-emerald-700">{autoMessage}</p> : null}
                </div>
              ) : null}
            </div>
          ) : null}
