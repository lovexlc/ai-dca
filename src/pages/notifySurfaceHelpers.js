export function assertNotifyTestDelivered(payload = {}, fallbackMessage = '测试通知发送失败') {
  // 异步投递模式：服务端已接收任务，最终送达结果写入通知历史。
  if (payload?.accepted === true && payload?.deferred === true) return;
  ...
  if (eventStatus && eventStatus !== 'delivered') throw new Error(String(failedChannel?.detail || fallbackMessage).trim() || fallbackMessage);
  if (deliveredCount <= 0) throw new Error(String(failedChannel?.detail || fallbackMessage).trim() || fallbackMessage);
}

export function detectNotifySurface() {
  if (typeof window === 'undefined') return { isNativeAndroid: false, isMobileWeb: false };
  ...
  const isMobileWeb = !isNativeAndroid && (/android|iphone|ipad|ipod|mobile/.test(userAgent) || (window.matchMedia?.('(pointer: coarse)')?.matches && window.matchMedia?.('(max-width: 900px)')?.matches));
...
export function getAvailableNotifyPlatforms(surface = {}) {
  ...
  if (surface.isMobileWeb) return [['ios', 'iOS'], ['serverchan3', 'Android'], ['email', 'Email']];
  return [['ios', 'iOS'], ['serverchan3', 'Android'], ['pc', 'PC 浏览器'], ['email', 'Email']];
}
