...
export function SwitchStrategyExperience({ links, inPagesDir = false, embedded = false, initialView = 'opportunity', hideViewTabs = false, mobileView = 'opportunity', mobileOnly = false, initialSymbol = '', entryAttribution = null, refreshToken = 0 } = {}) {
  ...
  return (
    <>
      {mobileOnly ? (mobileView === "opportunity" ? <MobileFundSwitchOpportunity fundsWithPremium={fundsWithPremium} heldCodes={exchangeFunds.map((fund) => fund.code).filter(Boolean)} intraSignals={intraSignals} workerSnapshot={activeWorkerSnapshot} workerError={workerStatus.error} otcSignal={otcSignal} prefs={prefs} navError={navState.error} navUpdatedHint={navUpdatedHint} workerConfig={workerConfig} onSetPrefValue={setPrefValue} onViewPlan={openMobilePlan} onEnableAutomation={() => { if (!workerConfig.enabled) handleWorkerToggle(true); }} /> : <MobileFundSwitchWatchlist prefs={prefs} fundsWithPremium={fundsWithPremium} workerConfig={workerConfig} onToggleWorker={handleWorkerToggle} onToggleRule={(ruleId, enabled) => { const next = normalizeSwitchConfigShape({ ...prefs, rules: (prefs.rules || []).map((rule) => rule.id === ruleId ? { ...rule, enabled: Boolean(enabled) } : rule) }); setPrefs(next); void persistWorkerConfig({ ...next, enabled: Boolean(workerConfig.enabled) }); }} />) : (
