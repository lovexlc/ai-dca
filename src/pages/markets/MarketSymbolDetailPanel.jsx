...
import { FundComparePkPanel } from './FundComparePkPanel.jsx';
import { FundCompareQuoteTable } from './FundCompareQuoteTable.jsx';
...
import { MARKET_EMPTY_VALUE, formatMarketPrice, formatNumber, formatPercent, formatSignedPercent, formatSymbolDisplay, normalizeCnFundCode } from './marketDisplayUtils.js';
import { dedupeCompareCandidates } from './marketOtcHelpers.js';
import { shouldEnableComparePk } from './marketDetailDataPolicy.js';
import { getCompareFromUrl, updateCompareInUrl, getChartConfigFromUrl, updateChartConfigInUrl } from './marketsUrlSync.js';
import { useFundComparePk } from './useFundComparePk.js';
...
function buildChartDataRangeSummary({ candles, navItems, cnFundParam, chartRange }) {
  ...
  const parts = [`图表 ${firstDate || MARKET_EMPTY_VALUE} 至 ${lastDate || MARKET_EMPTY_VALUE}`, `${arr.length} 个点`];
...
export function SymbolDetailPanel({
  ...
  // Skip clearing URL-hydrated compare on the first rowSymbol effect (deep-link).
  const skipCompareClearOnceRef = useRef(Boolean(getCompareFromUrl().length));
  ...
  const comparePkEnabled = shouldEnableComparePk({
    market,
    mainSymbol: rowSymbol,
    mainQuote: row,
    compareSymbols,
    compareQuoteMap,
    isMainOtc: currentIsCnOtcFund,
    premiumMode: premiumCompareMode,
  });
  const {
    feeMap: compareFeeMap,
    limitMap: compareLimitMap,
    pkQuoteMap: comparePkQuoteMap,
    loadingFees: comparePkLoadingFees,
    loadingLimits: comparePkLoadingLimits,
    loadingQuotes: comparePkLoadingQuotes,
    showLimits: comparePkShowLimits,
  } = useFundComparePk({
    market,
    mainRow: row,
    compareSymbols,
    compareQuoteMap,
    isMainOtc: currentIsCnOtcFund,
    enabled: comparePkEnabled,
  });
  // 主标的切换时清空对比；首次从 URL hydrate 时跳过一次，避免深链被清掉
  useEffect(() => {
    if (skipCompareClearOnceRef.current) {
      skipCompareClearOnceRef.current = false;
  ...
  const cnOtcFundExtras = market === 'cn' && isCnOtcFund ? [
    detailValueRow('今年以来', Number.isFinite(Number(row.ytdReturn)) ? formatSignedPercent(row.ytdReturn) : MARKET_EMPTY_VALUE),
    detailValueRow('最大回撤', Number.isFinite(Number(row.maxDrawdown)) ? formatSignedPercent(row.maxDrawdown) : MARKET_EMPTY_VALUE),
    detailValueRow('基金规模', formatCnMoney(row.fundSize)),
  ].filter((item) => item.value !== MARKET_EMPTY_VALUE) : [];
  const cnOverviewExtras = market === 'cn' && !isCnOtcFund ? [
    ...
    detailValueRow('溢价率', Number.isFinite(Number(xueqiuQuote?.premium_rate)) ? formatSignedPercent(xueqiuQuote?.premium_rate) : MARKET_EMPTY_VALUE),
    detailValueRow('年内涨幅', Number.isFinite(Number(xueqiuQuote?.current_year_percent)) ? formatSignedPercent(xueqiuQuote?.current_year_percent) : MARKET_EMPTY_VALUE),
  ...
  ].filter((item) => item.value !== MARKET_EMPTY_VALUE).slice(0, 18) : [];
  const overviewRows = [
    ...
    detailValueRow('涨跌额', Number.isFinite(change) ? `${change > 0 ? '+' : ''}${isCnOtcFund ? formatNumber(change) : formatMarketPrice(change, row)}` : MARKET_EMPTY_VALUE),
    ...
    <section className={cx('mx-0', backtestPanelOpen && 'lg:relative lg:h-full lg:min-h-0 lg:overflow-hidden')}>
      <div className={cx(
        ...
        <button
          type="button"
          onClick={onBack}
          className="mb-0.5 inline-flex items-center gap-1 text-xs font-medium text-[var(--market-text-muted)] hover:text-[var(--market-text-strong)] sm:mb-1 sm:text-[12px]"
        >
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            ...
            {isCnOtcFund && row.lastUpdated ? (
              <div className="mt-0.5 text-xs text-[var(--market-text-muted)] sm:text-[12px]">
            ...
            <div className="mt-0.5 flex flex-wrap items-baseline gap-x-1.5 gap-y-0.5 sm:gap-x-2">
              ...
              <span className={cx('text-[12px] font-medium tabular-nums sm:text-[13px]', positive ? 'text-[var(--market-rise)]' : negative ? 'text-[var(--market-fall)]' : 'text-[var(--market-text-muted)]')}>
                {Number.isFinite(change) ? `${change > 0 ? '+' : ''}${isCnOtcFund ? formatNumber(change) : formatMarketPrice(change, row)}` : MARKET_EMPTY_VALUE}
                <span className="mx-1 text-[var(--market-text-muted)]">·</span>
                {formatPercent(row.changePercent)}
              </span>
              <span className="text-xs text-[var(--market-text-muted)]">{isCnOtcFund ? '净值' : '今日'}</span>
            </div>
            {!isCnOtcFund ? (
              <div className="mt-0.5 flex flex-wrap items-center gap-x-1.5 gap-y-0.5 text-xs text-[var(--market-text-muted)] sm:text-xs">
            ...
            <button
              ...
              className={cx(
                ...
                inWatch
                  ...
                  : 'bg-[var(--market-text-strong)] text-white hover:bg-[#3c3c3c]'
              )}
            >
        ...
        {!summaryMode && onMarketAction ? (
          <div className="mt-2 flex items-center gap-1.5 overflow-x-auto pb-0.5 [scrollbar-width:none] sm:flex-wrap sm:overflow-visible sm:pb-0 [&::-webkit-scrollbar]:hidden">
            <button
              ...
              className="inline-flex h-8 shrink-0 items-center justify-center gap-1.5 rounded-full bg-[var(--market-text-strong)] px-3 text-[12px] font-semibold text-white transition hover:bg-[#3c3c3c] sm:h-9 sm:px-3.5 sm:text-[13px]"
              ...
              <div className="flex flex-col gap-0.5">
                {CHART_TYPE_OPTIONS.map((opt) => (
                  ...
                  >
                    ...
                    <span className="ml-auto text-xs text-[var(--market-text-subtle)]">{opt.hint}</span>
                  </button>
                ))}
              </div>
            )}
              ...
              {indicatorOptions.map((opt) => (
                <label key={opt.key} className="flex cursor-pointer items-center gap-2 rounded-lg px-2 py-1.5 hover:bg-[var(--market-surface-muted)]">
                  <input
                    ...
                    className="h-3.5 w-3.5 accent-[var(--market-accent)]"
                  />
            ...
            <div className="max-h-[420px] overflow-hidden text-[var(--market-text-strong)]">
              ...
              <div className="max-h-[344px] overflow-y-auto px-2.5 py-2">
                <div className="mb-1.5 flex items-center justify-between text-[12px] font-semibold text-[var(--market-text-muted)]">
                  <span>{compareInput ? '搜索结果' : (market === 'cn' ? '推荐对比基金' : '所有股票代码')}</span>
                    ...
                    return (
                      ...
                      >
                        ...
                        <div className="shrink-0 text-right">
                          <div className="text-[15px] font-semibold tabular-nums text-[var(--market-text-strong)]">{Number.isFinite(quote.price) ? `$${formatNumber(quote.price, 2)}` : MARKET_EMPTY_VALUE}</div>
        ...
        {/* 图表区 */}
          ...
          {premiumUnavailable ? (
            ...
            chartLoading || metricLoading ? (
              <div className="h-full w-full animate-pulse rounded-xl bg-gradient-to-r from-[var(--market-surface-muted)] via-white to-[var(--market-surface-muted)]" />
            ) : (
        ...
        {chartDataRangeSummary ? (
          <div
            ...
            className="mt-1.5 rounded-xl border border-[var(--market-border)] bg-white px-2.5 py-1.5 text-xs font-medium leading-4 text-[var(--market-text-muted)] sm:text-[12px]"
          >
            {chartDataRangeSummary}
          </div>
        ) : null}
              ...
              <div className="mt-2 flex items-center justify-end gap-1.5 sm:mt-0">
                ...
                <button
                  ...
                  className={cx(
                    ...
                    draftCustomRange
                      ? 'bg-[var(--market-text-strong)] text-white hover:bg-[#3c4043]'
        ...
        {premiumUnavailable ? (
          ...
          <FundCompareQuoteTable
            rows={displayCompareTableRows}
            market={market}
            premiumCompareMode={premiumCompareMode}
            premiumSpreadStats={premiumSpreadStats}
          />
        ) : null}
