    ...
    <div className="mt-3 rounded-xl border border-[var(--market-border)] bg-[var(--market-surface-subtle)] p-3">
      <div className="flex flex-wrap items-start justify-between gap-3">
        <div>
          ...
          {data.navDate ? <div className="mt-1 text-[11px] text-[var(--market-text-subtle)]">确认日期 {data.navDate}</div> : null}
        </div>
      ...
      <p className="mt-2 text-[11px] leading-4 text-[var(--market-text-subtle)]">净值取基金最新确认 NAV，场内基金盘中交易仍以价格为准。</p>
    </div>
  );
}
        ...
        <div className="grid grid-cols-5 gap-0 text-right text-[12px] sm:text-sm">
          ...
          {bidAskRows.map((it) => (
            <div key={it.level} className="contents">
              <div className="border-t border-[#f1f3f4] px-2 py-2 text-left text-[var(--market-text-muted)]">{it.level}档</div>
              <div className="border-t border-[#f1f3f4] px-2 py-2 tabular-nums text-[var(--market-text-strong)]">{formatNumber(it.bidPrice, 3)}</div>
              <div className="border-t border-[#f1f3f4] px-2 py-2 tabular-nums text-[var(--market-text-strong)]">{formatCnAmount(it.bidVolume)}</div>
              <div className="border-t border-[#f1f3f4] px-2 py-2 tabular-nums text-[var(--market-text-strong)]">{formatNumber(it.askPrice, 3)}</div>
              <div className="border-t border-[#f1f3f4] px-2 py-2 tabular-nums text-[var(--market-text-strong)]">{formatCnAmount(it.askVolume)}</div>
            </div>
          ))}
        </div>
      </div>
