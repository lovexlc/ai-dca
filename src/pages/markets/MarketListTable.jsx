  ...
  const columns = useMemo(() => ([
    ...
    },
    {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="space-y-0.5 text-[10px] leading-tight text-[var(--market-text-muted)]">
            {redeemTiers.split('\n').map((tier, idx) => (
      ...
      {viewPresets.map((preset) => {
        ...
        return (
          <span key={preset.id} className={cx(
            ...
            active
              ? 'border-[var(--market-accent)] bg-[var(--market-accent-soft)] text-[#174ea6] shadow-[inset_0_0_0_1px_rgba(26,115,232,0.12)]'
              : 'border-[var(--market-border-strong)] bg-white text-[var(--market-text-muted)]'
          )}>
            <button
              type="button"
              ...
              className={cx(
                ...
                active ? 'hover:bg-[#d2e3fc]' : 'hover:bg-[var(--market-surface-muted)]'
              )}
              title={preset.name}
            >
              {preset.name}
    ...
    <div className={cx('overflow-x-auto', compact ? 'rounded-xl border border-[var(--market-border)] bg-white' : 'rounded-2xl border border-[var(--market-border)] bg-white shadow-sm')}>
      <div ref={menuRef} className="relative flex items-center justify-end px-2 py-1">
        <button
          ...
          className="inline-flex h-7 items-center gap-1 rounded-md px-2 text-[11px] text-[var(--market-text-muted)] hover:bg-[var(--market-surface-muted)]"
        >
            ...
            return (
              ...
              >
                ...
                <td className={cx(cellPad, 'min-w-[120px] text-[var(--market-text-strong)]')}>
                  ...
                  {row.meta ? <div className="truncate text-[10px] text-[var(--market-text-muted)]">{row.meta}</div> : null}
                </td>
                ...
                <td className={cx(cellPad, 'whitespace-nowrap text-center')}>
                  <span className="inline-flex items-center justify-center gap-1.5">
                    ...
                    {isLatestChangeRow(row) ? <span className="rounded-full bg-[var(--market-accent-soft)] px-1.5 py-0.5 text-[10px] font-semibold leading-none text-[var(--market-accent)]">最新</span> : null}
                  </span>
                </td>
