          ...
          {searchResults.map((row) => {
            ...
            return (
              <li key={`${row.market || marketLabel}:${row.symbol}:${venueKey}`} className="flex items-center gap-3 px-3 py-2 hover:bg-white">
                <button type="button" className="min-w-0 flex-1 text-left" onClick={() => onSearchResultSelect?.(row)}>
                ...
                {showAddButton ? (
                  <button
                    ...
                    className={cx('shrink-0 rounded-full px-3 py-1 text-xs font-semibold transition', alreadyAdded ? 'bg-[var(--market-border)] text-[var(--market-text-subtle)]' : 'bg-[var(--market-accent-soft)] text-[var(--market-accent)] hover:bg-[var(--market-accent-soft)]')}
                  >
  ...
  return (
    <div className={cx(
      ...
      <div className="flex items-center gap-2 rounded-full bg-[var(--market-surface-muted)] px-3 py-2">
        ...
        />
        {searchValue ? (
          <button type="button" aria-label="清空搜索" onClick={onSearchClear} className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[var(--market-text-muted)] hover:bg-white"><X size={14} /></button>
        ) : null}
      </div>
      {hasQuery ? inline ? (
        <div className="absolute left-0 top-full z-30 w-[min(22rem,calc(100vw-1rem))] max-w-[calc(100vw-1rem)] sm:w-full sm:min-w-[320px]">{resultPanel}</div>
      ) : resultPanel : inline ? null : (
        <div className="mt-3 rounded-2xl border border-dashed border-[var(--market-border)] bg-[var(--market-surface-subtle)] px-3 py-3 text-sm text-[var(--market-text-muted)]">输入基金代码或名称，搜索后可直接加入当前自选列表。</div>
      )}
    </div>
  );
}
