              ...
              <li key={`${row.market || marketLabel}:${row.symbol}:${venueKey}`} className="flex items-center gap-3 px-3 py-2 hover:bg-white">
                ...
                {showAddButton ? (
                  <button
                    ...
                    className={cx('shrink-0 rounded-full px-3 py-1 text-xs font-semibold transition', alreadyAdded ? 'bg-[#e8eaed] text-[#9aa0a6]' : 'bg-[#e8f0fe] text-[#1a73e8] hover:bg-[#d2e3fc]')}
                  >
  ...
  return (
    <div className={cx(
      ...
      <div className="flex items-center gap-2 rounded-full bg-[#f1f3f4] px-3 py-2">
        ...
        />
        {searchValue ? (
          <button type="button" aria-label="清空搜索" onClick={onSearchClear} className="inline-flex h-7 w-7 shrink-0 items-center justify-center rounded-full text-[#5f6368] hover:bg-white"><X size={14} /></button>
        ) : null}
      </div>
      {hasQuery ? inline ? (
        ...
        <div className="mt-3 rounded-2xl border border-dashed border-[#e8eaed] bg-[#f8fafd] px-3 py-3 text-sm text-[#5f6368]">输入基金代码或名称，搜索后可直接加入当前自选列表。</div>
      )}
    </div>
  );
}
