  ...
  if (showPremiumDistribution) {
    return (
      <div className="grid h-full w-full grid-cols-[minmax(0,1fr)_108px] items-center gap-1 px-1 sm:grid-cols-[minmax(0,1fr)_148px] sm:gap-2 sm:px-2">
        ...
        <div className="min-w-0 pr-1 text-[11px] font-medium text-[var(--market-text-muted)] sm:text-[12px]">
          ...
          <div className="mt-1 text-[10px] text-[var(--market-text-subtle)] sm:text-[11px]">样本 {premiumDistributionTotal}</div>
        </div>
      </div>
    );
  }
      ...
      {canRenderResponsiveChart ? (
        ...
        {visibleTradeMarkerPoints.map((marker) => (
          <ReferenceDot
            ...
            ifOverflow="visible"
            isFront
            label={marker.label ? { value: marker.label, position: marker.type === 'BUY' ? 'bottom' : 'top', fill: marker.color, fontSize: 12, fontWeight: 700 } : undefined}
          />
        ))}
