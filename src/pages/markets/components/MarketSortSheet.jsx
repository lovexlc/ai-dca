...
export function MarketSortSheet({ open, sorting, onApply, onClose, isOtc = false }) {
  ...
  return createPortal(
    <div className="market-sheet-backdrop" role="dialog" aria-modal="true" aria-label="排序条件" onMouseDown={onClose}>
      <section className="market-filter-sheet market-sort-sheet" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="market-sort-sheet__section"><h3>排序字段</h3><div className="market-sort-options">{availableOptions.map(([id, label]) => <button type="button" key={id} className={primary.id === id ? 'is-active' : ''} onClick={() => setPrimary(id)}>{label}</button>)}</div></div>
        ...
        <div className="market-sort-sheet__section"><h3>次级排序</h3><div className="market-sort-options">{availableSecondaryOptions.map(([id, label]) => <button type="button" key={label} className={(secondary.id || '') === id ? 'is-active' : ''} onClick={() => setSecondary(id)}>{label}</button>)}</div></div>
