...
import buyMeCoffeeQr from '../assets/buy-me-coffee-qr.jpg';
...
const PRESERVED_QUERY_PARAMS_BY_TAB = {
  home: ['view', 'currency'],
  markets: ['view', 'symbol', 'compare', 'chartType', 'indicators', 'cnFundParam', 'chartRange', 'chartFrom', 'chartTo'],
  ...
  return (
    <>
      ...
      {(tabHistory.length > 0 || showScrollTop) ? (
        <div className="fixed bottom-[calc(52px+env(safe-area-inset-bottom)+1.5rem)] right-4 z-40 flex flex-col gap-2 sm:bottom-8 sm:right-6" aria-label="页面快捷操作">
          {tabHistory.length > 0 ? (
            <button
              type="button"
      ...
      {showQrModal ? (
        <div className="fixed inset-0 z-[110] flex items-center justify-center bg-slate-900/70 p-4" role="dialog" aria-modal="true" aria-label="Buy me coffee 二维码" onClick={() => setShowQrModal(false)}>
          <div className="relative w-full max-w-sm" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="overflow-hidden rounded-2xl bg-white shadow-2xl">
              <img src={buyMeCoffeeQr} alt="Buy me coffee 二维码" className="block w-full" />
              <p className="px-4 py-3 text-center text-xs text-slate-600">扫码支持作者</p>
            </div>
          </div>
        </div>
      ) : null}
