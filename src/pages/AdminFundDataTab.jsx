    ...
    <div className={cx('space-y-4', embedded ? '' : 'px-6')}>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-4 shadow-sm">
        <div className="flex flex-col gap-2 lg:flex-row lg:items-center">
          <input value={query} onChange={(event) => setQuery(event.target.value)} onKeyDown={(event) => { if (event.key === 'Enter') { setPage(1); setAppliedQuery(query.trim()); } }} placeholder="搜索基金代码或名称" className="min-w-0 flex-1 rounded-xl border border-slate-200 px-3 py-2 text-sm outline-none focus:border-indigo-400" />
          <button type="button" onClick={() => { setPage(1); setAppliedQuery(query.trim()); }} className="rounded-xl bg-indigo-600 px-4 py-2 text-sm font-semibold text-white hover:bg-indigo-500">搜索</button>
      ...
      {editor ? <section className="rounded-3xl border border-indigo-200 bg-indigo-50/40 p-4 shadow-sm"><div className="flex items-center justify-between"><div><h3 className="text-base font-bold text-slate-900">{payload.items.some((item) => item.code === editor.code) ? '编辑基金数据' : '新增基金数据'}</h3><p className="mt-1 text-xs text-slate-500">费率数字按百分比填写，例如 0.5 表示 0.5%。</p></div><button type="button" onClick={() => setEditor(null)} className="rounded-lg p-1.5 text-slate-400 hover:bg-white"><X className="h-4 w-4" /></button></div><div className="mt-4 grid gap-3 sm:grid-cols-2 lg:grid-cols-3">{[['code', '基金代码'], ['name', '基金名称'], ['fundType', '基金类型'], ...RATE_FIELDS].map(([key, label]) => <label key={key} className="text-xs font-semibold text-slate-600">{label}<input value={editor[key] || ''} disabled={key === 'code' && payload.items.some((item) => item.code === editor.code)} onChange={(event) => setEditor((prev) => ({ ...prev, [key]: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-normal outline-none focus:border-indigo-400" /></label>)}</div><div className="mt-3 grid gap-3 lg:grid-cols-3">{[['purchaseRules', '申购费率规则 JSON'], ['redeemRules', '赎回费率规则 JSON'], ['operationFees', '运作费用规则 JSON']].map(([key, label]) => <label key={key} className="text-xs font-semibold text-slate-600">{label}<textarea rows={5} value={editor[key] || ''} onChange={(event) => setEditor((prev) => ({ ...prev, [key]: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 font-mono text-xs font-normal outline-none focus:border-indigo-400" placeholder="可留空；填写 JSON 数组" /></label>)}</div><label className="mt-3 block text-xs font-semibold text-slate-600">备注<textarea rows={2} value={editor.notice || ''} onChange={(event) => setEditor((prev) => ({ ...prev, notice: event.target.value }))} className="mt-1 w-full rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-normal outline-none focus:border-indigo-400" /></label>{saveError ? <div className="mt-3 rounded-xl bg-rose-50 px-3 py-2 text-sm text-rose-700">{saveError}</div> : null}<div className="mt-4 flex justify-end gap-2"><button type="button" onClick={() => setEditor(null)} className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-600">取消</button><button type="button" disabled={saving || !/^\d{6}$/.test(editor.code)} onClick={save} className="inline-flex items-center gap-1.5 rounded-xl bg-indigo-600 px-3 py-2 text-sm font-semibold text-white disabled:opacity-50"><Save className="h-4 w-4" />{saving ? '保存中…' : '保存到 D1'}</button></div></section> : null}
    </div>
  );
}
