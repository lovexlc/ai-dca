...
function RecommendationView({ recommendation, fee, onBack, onUse, onBacktest }) {
  ...
  const selectionIsOptimized = backtest.selectionStatus === 'optimized';
  return <Panel>
    ...
    <div className="mt-5 rounded-xl border border-slate-100 p-4"><div className="flex items-center justify-between"><h3 className="font-bold text-slate-900">历史回测</h3><span className={cx('rounded-full px-2.5 py-1 text-xs font-semibold', backtest.status !== 'passed' ? 'bg-amber-50 text-amber-700' : selectionIsOptimized ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-100 text-slate-600')}>{backtest.status !== 'passed' ? '数据不足' : selectionIsOptimized ? '自动推荐' : '参考值'}</span></div><div className="mt-4 grid grid-cols-2 gap-4 sm:grid-cols-4"><Stat label="触发" value={`${backtest.triggerCount ?? 0} 次`} /><Stat label="胜率" value={formatPercent(backtest.winRatePct, 1)} tone="emerald" /><Stat label="年化提升" value={formatPercent(backtest.annualizedReturnPct, 1)} tone="emerald" /><Stat label="最大回撤" value={formatPercent(backtest.maxDrawdownPct, 2)} /></div>{!selectionIsOptimized && backtest.selectionReason ? <div className="mt-4 rounded-lg bg-slate-50 p-3 text-xs leading-5 text-slate-500">{backtest.selectionReason}</div> : null}</div>
