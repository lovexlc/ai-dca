...
export function CnHomeExperience() {
  ...
  return <main className="cn-home"><div className="cn-home-head"><div><h1>市场首页</h1><p>全球指数基金实时概览</p></div><button onClick={() => refresh(true)} disabled={Object.values(state).some((x) => x.loading)}><RefreshCw />刷新</button></div><Section className="cn-home-overview" title="全局市场总览" loading={state.overview.loading} error={state.overview.error} onRetry={() => refresh(true)}><Overview data={state.overview.data} limits={state.limits.data} /></Section><Section className="cn-home-market" title="场内走势" loading={state.series.loading} error={state.series.error} onRetry={() => refresh(true)} side={<div className="cn-home-market-toolbar"><span className="cn-home-series-meta">{state.series.data?.windowLabel || '今日 · 5 分钟'} · {timeText(state.series.data?.generatedAt)}</span><div className="cn-home-segment" role="group" aria-label="走势指标"><button type="button" aria-pressed={mode === 'premium'} className={mode === 'premium' ? 'active' : ''} onClick={() => setMode('premium')}>溢价</button><button type="button" aria-pressed={mode === 'price'} className={mode === 'price' ? 'active' : ''} onClick={() => setMode('price')}>价格</button></div></div>}><div className="cn-home-chips">{groups.map((item) => <button type="button" key={item.key} aria-pressed={group === item.key} className={group === item.key ? 'active' : ''} onClick={() => setGroup(item.key)}>{item.label}</button>)}</div><MiniChart series={lines} mode={mode} /></Section><Section className="cn-home-limits" title="场外额度" loading={state.limits.loading} error={state.limits.error} onRetry={() => refresh(true)} side={<span className="cn-home-limit-asof">更新于 {timeText(state.limits.data?.limitAsOf || state.limits.data?.generatedAt)}</span>}><Limits data={state.limits.data} onFund={openFund} onOpenDetail={openLimitDetail} /></Section></main>;
}
