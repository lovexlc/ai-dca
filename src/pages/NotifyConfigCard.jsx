...
export function NotifyConfigCard({
  ...
  return (
    <Card className="min-w-0 overflow-hidden !p-0 sm:!p-0">
      <div className="bg-gradient-to-br from-slate-950 via-slate-900 to-indigo-950 px-5 py-5 text-white sm:px-6 sm:py-6">
        <div className="flex flex-col gap-5 lg:flex-row lg:items-center lg:justify-between">
          <div className="flex min-w-0 items-start gap-3">
            <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-white/10 text-indigo-200 ring-1 ring-white/10">
              <Bell className="h-5 w-5" />
            </div>
            <div className="min-w-0">
              <div className="flex flex-wrap items-center gap-2">
                <h1 className="text-xl font-bold tracking-tight sm:text-2xl">通知管理</h1>
                <span className={cx(
                  'inline-flex items-center gap-1.5 rounded-full px-2.5 py-1 text-[11px] font-semibold',
                  hasAnyChannel ? 'bg-emerald-400/15 text-emerald-200 ring-1 ring-emerald-300/20' : 'bg-white/10 text-slate-300 ring-1 ring-white/10'
                )}>
                  <span className={cx('h-1.5 w-1.5 rounded-full', hasAnyChannel ? 'bg-emerald-300' : 'bg-slate-400')} />
                  {hasAnyChannel ? '推送已就绪' : '等待接入渠道'}
                </span>
              </div>
              <p className="mt-2 max-w-2xl text-sm leading-6 text-slate-300">
                先连接至少一个接收渠道，再管理提醒规则与送达记录。所有云端配置均跟随当前登录账号。
              </p>
            </div>
          </div>
          <div className="flex w-full items-center gap-4 rounded-2xl bg-white/5 px-4 py-3 ring-1 ring-white/10 lg:w-auto lg:min-w-[270px]">
            <div className="min-w-0 flex-1">
              <div className="flex items-center justify-between gap-4 text-xs text-slate-300">
                <span>可用渠道</span>
                <strong className="text-sm text-white">{connectedChannelCount} / {channelCards.length}</strong>
              </div>
              <div className="mt-2 h-1.5 overflow-hidden rounded-full bg-white/10">
                <div className="h-full rounded-full bg-emerald-400 transition-all" style={{ width: `${connectionProgress}%` }} />
              </div>
            </div>
            <button
              type="button"
              className="inline-flex min-h-9 items-center gap-1.5 rounded-xl bg-white/10 px-3 text-xs font-semibold text-white ring-1 ring-white/10 transition-colors hover:bg-white/15"
            ...
            >
              ...
              {isConfigCollapsed ? '管理渠道' : '收起详情'}
            </button>
          </div>
        </div>
      </div>

      <div className="grid gap-3 border-b border-slate-100 bg-slate-50/70 p-4 sm:grid-cols-2 sm:p-5 xl:grid-cols-4" role="tablist" aria-label="通知渠道">
        {channelCards.map((channel) => {
          ...
          return (
            <button
              ...
              className={cx(
                'group flex min-w-0 items-center gap-3 rounded-2xl border px-3.5 py-3 text-left transition-all focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-indigo-300',
                selected
      ...
      {!isConfigCollapsed ? (
        <div id="notify-channel-panel" role="tabpanel" className="space-y-5 px-4 py-5 sm:px-6 sm:py-6">
          ...
          {notifyError ? (
            <div role="alert" className="rounded-2xl border border-rose-200 bg-rose-50 px-4 py-3 text-sm text-rose-700">{notifyError}</div>
          ) : null}
          {notifyMessage ? (
            <div role="status" className="flex items-start gap-2 rounded-2xl border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm text-emerald-700">
