...
**commit**: `36abcb1` (e0f8705..36abcb1, +207/-512 LOC)  
**Actions**: `25988991557` success 32s @ 2026-05-17T11:00:28Z  
**Pages last-modified**: 2026-05-17T11:00:57Z

### 落地清单
- 5.1 ✅ `IncomeSummary.jsx` 完全重写：17205B → 4501B（-74%）。总市值大字 + 当日行 + 累计行 + 5 tile（8 镜头 → 3 镜头 + 5 入口）
- 5.2 ✅ `HoldingsExperience.renderPortfolioOverview()`：删 10 KPI cards 数组 (L1875-1904) + cards.map grid (L1932-1959) + 55 行死变量 (profitTone/cumulativeTone/todayTone/navIncomplete/navBadge/lastUpdateDisplay/toneClass)。保留 section header “投资组合概览” label + 行情中心入口 + ticker
- 5.3 ✅ `IncomeSection`：签名增加 `portfolio` + `inceptionDate`。OVERVIEW 分支转发给 IncomeSummary
- 5.4 ✅ `portfolioSeries.annualize()`：加护栏 `if (days < 365) return null` — 修 137 天 +559% → +15729.41% 误导 bug
- 5.5 ✅ `HoldingsExperience` 加 `inceptionDate` useMemo (L203, 以 transactions 中最早 BUY 日期)
- 5.6 ✅ ESLint：0 error，0 新增 warning

### 预期效果
主页“持仓总览”顶部 “投资组合概览” 区场 10 KPI cards 全消失，只留一条“投资组合概览 / 行情中心 ┚ / 行情 ticker”薄条。主页重心上移到 “收益 OVERVIEW” 区，渲染 “总市值 3xl + 当日·累计 · 5 tile 入口” — 接近支付宝基金列表首页信息密度。

### 限制
- cf-browser-mcp 在 script 模式不可用，未能提供截图证据（同 v2 降级原因）。用 Actions success + Pages last-modified 代证。
