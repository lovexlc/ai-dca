...
## 热修记录：A股 ETF K 线兜底
- 问题：`513100` / `513500` 报价已恢复，但东方财富 `push2his` 历史 K 线接口在 Worker 侧返回 520，前端仍显示暂无趋势数据。
- 修复：新增新浪 K 线备用源；东方财富 K 线失败时自动 fallback 到新浪 `CN_MarketDataService.getKLineData`。
- 验证：本地直连新浪接口确认 `sh513100` / `sh513500` 均能返回日线数据。

## 功能记录：A股 ETF 溢价指标
- 需求：A股对应基金的「指标」中增加「溢价」，点击后展示对应基金溢价情况。
- 计算口径：`iopv = 上一工作日的净值 × (1 + QQQ涨幅)`；`溢价 = (价格 − iopv) / iopv`。
- 实现：A股详情页图表工具栏的「指标」下新增「溢价」；选中后展示价格、上一工作日净值、QQQ 涨幅、估算 IOPV 和溢价率，并展开公式明细。
- 数据源：基金净值沿用 `/api/holdings/nav` 的最新确认 NAV；QQQ 涨幅走行情 Worker 的 `/api/markets/quote/QQQ`；A股 ETF 当前价格沿用行情页已加载报价。
