...
普通页面不得展示 H/L、规则 A/B、H-L 公式、premiumClass、intraSellLowerPct、intraBuyOtherPct、Worker 输出或 Cron 表达式。
...
保存到 Worker 时必须同时写入 benchmarkCodes、enabledCodes、premiumClass、runtimeConfig.premiumClass、intraSellLowerPct、intraBuyOtherPct、thresholdMode 和 thresholdValue。

用户只编辑 thresholdValue；另一个未被编辑的运行时阈值必须保留，不能把两个 intra* 字段写成同一个值。
...
- workers/notify/src/switchRecommendation.js：候选池筛选、调用当前分类函数、历史数据准备、回测场景、推荐阈值选择、推荐结果组装和推荐输入哈希；
- workers/notify/src/switchStrategy.js：规则归一化、运行时分类刷新、快照、触发判断、状态去重和通知构建；
...
1. 优先沿用上次持久化的 premiumClass
2. 设置 classificationStatus = stale
3. 设置 classificationWarning
4. 写 warning 日志
5. 继续使用旧分类计算快照和正式提醒
6. 只有没有完整历史分类时才设置 pending_classification

stale 正式通知正文追加：

~~~text
本次提醒基于上次历史分析结果，建议打开 App 确认当前数据。
~~~
...
- lte 规则快捷值 0.1% 至 2%，步长 0.1%，并提供 0.5% 的常用快捷值
- 支持自定义
- 范围校验
- 根据 gte/lte 生成用户文案
...
在 validateFeeConfig() 同级增加 validateThresholdValue(value, operator)，并在“生成推荐规则”“保存规则”“采用回测值”三个入口都调用。阈值校验失败时保留用户输入并显示字段级错误，不发起保存或推荐请求。阈值范围必须按操作符校验：gte 为 0.5% 至 5%，lte 为 0.1% 至 2%；负数、空值和超范围值均拒绝。
